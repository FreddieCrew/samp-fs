...
CMD:createdir(playerid, params[]) {
	...
	if (fs_dexists(name)) return SendClientMessage(playerid, -1, "Error: This directory already exists");
...
CMD:deldir(playerid, params[]) {
	...
	if (!fs_dexists(name)) return SendClientMessage(playerid, -1, "Error: This directory does not exist.");
...
CMD:direxists(playerid, params[]) {
	new path[MAX_PATH];

	if (sscanf(params, "s[256]", path)) return SendClientMessage(playerid, -1, "/direxists [path]");

	if (fs_dexists(path)) {
		return SendClientMessage(playerid, -1, "The directory exists!");
	} else {
		SendClientMessage(playerid, -1, "The directory does not exist :(");
	}
	return 1;
}